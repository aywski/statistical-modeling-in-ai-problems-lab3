...
def plot_advanced_results(results_ht, results_hh, balances_ht, balances_hh, wins_ht, wins_hh):
    ...
    plt.legend()
    plt.savefig("win_distribution.png")  # Збереження гістограми виграшів

    # 2. Boxplot виграшів
    plt.subplot(3, 2, 2)
    box_data = [results_ht, results_hh]
    plt.boxplot(box_data, labels=['Орел-Решка', 'Орел-Орел'])
    plt.title('Порівняння розподілів виграшів')
    plt.ylabel('Виграш ($)')
    plt.savefig("boxplot_wins.png")  # Збереження boxplot
    ...
    plt.ylabel('Баланс ($)')
    plt.legend()
    plt.savefig("balance_change.png")  # Збереження графіка зміни балансу
    ...
    plt.ylabel('Кількість гравців')
    plt.legend()
    plt.savefig("win_combinations.png")  # Збереження гістограми виграшних комбінацій

    # 5. Q-Q plots для "Орел-Решка"
    plt.subplot(3, 2, 5)
    stats.probplot(results_ht, dist="norm", plot=plt)
    plt.title('Q-Q plot (Орел-Решка)')
    plt.savefig("qq_plot_head_tail.png")  # Збереження Q-Q plot для "Орел-Решка"

    # 6. Q-Q plots для "Орел-Орел"
    plt.subplot(3, 2, 6)
    stats.probplot(results_hh, dist="norm", plot=plt)
    plt.title('Q-Q plot (Орел-Орел)')
    plt.savefig("qq_plot_head_head.png")  # Збереження Q-Q plot для "Орел-Орел"

    plt.tight_layout()
    plt.close()  # Закриває фігуру після збереження всіх графіків
